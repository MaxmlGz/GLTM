#include "wrapper_python.h"   // Class container
#include "DataTypes.h"        // Headers for main method
#include "GeneralFunctions.h" // Headers for utility functions
#include <iostream>           // For outputs

outputs wrapperPython::verbose_sampler_function(gsl_matrix *X, int *C, int *R, gsl_matrix *paramW, double s2Z, double s2B, double s2Y, double s2u, double s2theta, int Nsim, int maxK, gsl_matrix *XT)
{
    printf("Inside the C++ routine.\n");

    // Get array shape: recall this has already been transposed
    int D = X->size1;
    int N = X->size2;
    ...
    if (D > N)
    {
        printf("Are you sure the observation matrix X has been transposed? It has more dimensions (D = %i) than example objects (N = %i).", D, N);
        exit(EXIT_SUCCESS);
    }
    ...
    for (int d = 0; d < D; d++)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            alpha_W[d][i] = gsl_matrix_get(paramW, d, i);
        }
    }
    // Get array shape of the "complete version"
    int D1 = XT->size1;
    int N1 = XT->size2;
    ...
    //     {
    //         double tdn, xdn;
    //         tdn = gsl_matrix_get(XT, d, n);
    //         xdn = gsl_matrix_get(X, d, n);
    //         printf("XT entry: %f and X entry: %f\n",tdn,xdn);
    //     }
    // }
    // Everything below is actual functionality.
    ...
    for (int d = 0; d < D; d++)
    {
        ...
        {
            S[n][d] = 0;
            double xnd = gsl_matrix_get(X, d, n); // This shape is already in transposed form.
            if (xnd != -1 && xnd > maxX[d])
            {
                maxX[d] = xnd;
            }
    ...
    for (int d = 0; d < D; d++)
    {
        ...
        { //Continuous without Positive Real
            double xnd;
            int snd;
            //             Y[d] =gsl_matrix_calloc(R[d],N);
            Yreal[d] = gsl_matrix_calloc(1, N);
        ...
        { //Discrete
            ...
            for (int r = 1; r < R[d]; r++)
            {
                ...
                if (r < R[d] - 1)
                {
                    gsl_vector_set(theta[d], r, gsl_vector_get(theta[d], r - 1) + (4 * sY / maxX[d]) * drand48());
                }
            }
            ...
            for (int d = 0; d < D; d++)
            {
                ...
                else if (C[d] == 4)
                {
                    ...
                    if (r == -1 || isnan(r))
                    {
                        ...
                        {
                            if (gsl_matrix_get(Ycat[d], r2, n) > auxY)
                            {
                                r = r2;
                            }
                        }
        ...
        for (int d = 0; d < D; d++)
        {
            ...
            else if (C[d] == 4)
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    ...
                    { //Ordinal
                        ...
                        else if (xnd == R[d])
                        {
                            ...
                            if (gsl_matrix_get(Yord[d], 0, n) < gsl_vector_get(Ymin, xnd - 2))
                            {
                                gsl_vector_set(Ymin, xnd - 2, gsl_matrix_get(Yord[d], 0, n));
                            }
                        }
                        else
                        {
                            ...
                            if (gsl_matrix_get(Yord[d], 0, n) > gsl_vector_get(Ymax, xnd - 1))
                            {
                                gsl_vector_set(Ymax, xnd - 1, gsl_matrix_get(Yord[d], 0, n));
                            }
                            if (gsl_matrix_get(Yord[d], 0, n) < gsl_vector_get(Ymin, xnd - 2))
                            {
                                gsl_vector_set(Ymin, xnd - 2, gsl_matrix_get(Yord[d], 0, n));
                ...
                for (int r = 1; r < R[d] - 1; r++)
                {
                    double xlo;
                    double xhi;
                    if (gsl_vector_get(theta[d], r - 1) > gsl_vector_get(Ymax, r))
                    {
                        xlo = gsl_vector_get(theta[d], r - 1);
                    }
                    else
                    {
                        xlo = gsl_vector_get(Ymax, r);
                    }
                    if (gsl_vector_get(theta[d], r + 1) < gsl_vector_get(Ymin, r))
                    {
                        xhi = gsl_vector_get(theta[d], r + 1);
                    }
                    else
                    {
                        xhi = gsl_vector_get(Ymin, r);
                    }
                    if (r == R[d] - 2)
                    {
                        gsl_vector_set(theta[d], r, truncnormrnd(0, s2theta, xlo, GSL_POSINF));
                    }
                    else
                    {
                        gsl_vector_set(theta[d], r, truncnormrnd(0, s2theta, xlo, xhi));
                    }
                }
            ...
            else if (C[d] == 2)
            {
                ...
                for (int n = 0; n < N; n++)
                {
                    ...
                    else
                    {
                        ...
                        Bd_view = gsl_matrix_submatrix(Bint[d], 0, 0, Kest, 1);
                        matrix_multiply(&Zn_view.matrix, &Bd_view.matrix, aux, 1, 0, CblasTrans, CblasNoTrans);
                        p[1] = p[1] + xpdf_int(xnd, Wint, 2 / (maxX[d] - meanX[d]), meanX[d], gsl_matrix_get(aux, 0, 0), s2Y, s2u);
                        //printf("p(%d)=%f ",1, p[1]);
                        p[1] = p[1] + logFun(W[d][1]);
    ...
    // Call the pre-defined output structure
    outputs results;

    // Store for reference even though it does not change
    results.Kest = Kest;
    // Check if any errors during run
    results.countErr = countErr + countErrC;

    // Get weights
    gsl_matrix *out_West = gsl_matrix_calloc(D, 4);
    for (int d = 0; d < D; d++)
    {
        for (int i = 0; i < 4; i++)
        {
            gsl_matrix_set(out_West, d, i, W[d][i]);
        }
    }

    // Get likelihoods
    gsl_matrix *out_LIK = gsl_matrix_calloc(D, N);
    for (int d = 0; d < D; d++)
    {
        for (int n = 0; n < N; n++)
        {
            gsl_matrix_set(out_LIK, d, n, LIK[d][n]);
        }
    }

    // Save to struct and return
    results.West = *out_West;
    results.LIK = *out_LIK;

    return results;

    // Free output related memory
    gsl_matrix_free(out_West);
    gsl_matrix_free(out_LIK);
...
void wrapperPython::sampler_function(gsl_matrix *X, int *C, int *R, gsl_matrix *paramW, double s2Z, double s2B, double s2Y, double s2u, double s2theta, int Nsim, int maxK, gsl_matrix *XT)
{

    // Remember that the following are simply pre-processing directives and they are not
    // seen by the compiler, so it does not matter that we stick them in the main body.

    //*********************************OUTPUTS**************************//

    // We write these as pure C++ instead, since we are not interestd in Matlab -type
    // outputs.
    ...
    printf("I got this far, line 52.\n");

    // OBSERVE ::::: the X matrix has already been transposed at this point.

    int D = X->size1; // Rows
    int N = X->size2; // Columns
    ...
    // This is the old piece of code we should not need IF we already pass Xview.matrix
